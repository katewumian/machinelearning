...
#apply to extract 2000 best related gene
feature_matrix_dataframe_new_np = SelectKBest(chi2, k=2000).fit_transform(X, y)
...
#合并
all_in = pd.concat([feature_matrix_dataframe_new,feature_matrix_dataframe_test],axis=0,join='inner')
...
new_train = new_all.iloc[:2000,]  #新的train数据 
new_test = new_all.iloc[2000: ,]  #新的test数据
